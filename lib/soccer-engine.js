 ...
 *
 * Data Model:
 * - Aggregates stored as: matchId -> roomKey -> period -> sliceIndex -> { counts, total, lastTapSeqByEmotion }
...
class SoccerEngine {
  constructor(
    ...
    this._listArchive = listArchiveFn;
    this._listTapEvents = listTapEventsFn;
    ...
    // Monotonic sequence for deterministic "last tap wins" tie-breaks.
    this._tapSequence = 0;
  ...
  // Rehydrate archived matches and aggregates into memory on server boot.
  async restoreArchivedMatches() {
    if (!this._listArchive || !this._getArchive) return 0;

    const snapshots = await this._listArchive();
    if (!Array.isArray(snapshots) || snapshots.length === 0) return 0;

    let restored = 0;
    for (const snap of snapshots) {
      const archiveId = String(snap.id || '');
      if (!archiveId.startsWith('soccer-')) continue;
      const matchId = archiveId.replace(/^soccer-/, '');
      const existing = this.matches.get(matchId);
      if (existing && existing.archived) continue;

      const full = await this._getArchive(archiveId);
      if (!full || !full.payload) continue;

      const payload = full.payload;
      const homeTeam = payload.homeTeam || { id: null, name: 'Home' };
      const awayTeam = payload.awayTeam || { id: null, name: 'Away' };
      this.matches.set(matchId, {
        matchId,
        status: STATUS.ENDED,
        elapsedSeconds: payload.elapsedSeconds || 0,
        currentPeriod: PERIOD.ENDED,
        currentSliceIndex: 0,
        homeTeam,
        awayTeam,
        startTime: payload.startTime || null,
        periodLabel: PERIOD.ENDED,
        archived: true,
        worldCupRoomsEnabled: this.isWorldCupRoomsEnabled(matchId),
        hasExtraTime: !!payload.hasExtraTime,
        hasPenalties: !!payload.hasPenalties,
        penaltyKicks: 0,
      });

      const matchAgg = new Map();
      const payloadPeriods = payload.periods || {};
      for (const [roomKey, roomPeriods] of Object.entries(payloadPeriods)) {
        const roomAgg = new Map();
        for (const [period, periodSlices] of Object.entries(roomPeriods || {})) {
          const periodAgg = new Map();
          for (const [sliceIndex, agg] of Object.entries(periodSlices || {})) {
            periodAgg.set(Number(sliceIndex), {
              counts: { ...(agg.counts || this._emptyCounts()) },
              total: agg.total || 0,
              lastTapSeqByEmotion: { ...(agg.lastTapSeqByEmotion || {}) },
            });
          }
          roomAgg.set(period, periodAgg);
        }
        matchAgg.set(roomKey, roomAgg);
      }
      this.aggregates.set(matchId, matchAgg);
      restored++;
    }

    if (restored > 0) {
      console.log(`[SoccerEngine] Restored ${restored} archived matches from persistence.`);
    }
    return restored;
  }
  ...
  _dominantOf(counts, lastTapSeqByEmotion = {}) {
    const emotions = this._getEmotions();
    ...
    for (const e of emotions) {
      ...
      const seq = Number(lastTapSeqByEmotion[e.key] || -1);
  ...
  _getSliceAggregate(matchId, roomKey, period, sliceIndex) {
    ...
    if (!periodAgg.has(sliceIndex)) {
      periodAgg.set(sliceIndex, {
        counts: this._emptyCounts(),
        total: 0,
        lastTapSeqByEmotion: {},
      });
    }
  ...
  recordTap(matchId, roomKey, period, sliceIndex, emotion) {
    ...
    agg.total++;
    this._tapSequence++;
    agg.lastTapSeqByEmotion[emotion] = this._tapSequence;
    return {
      period,
      sliceIndex,
      counts: agg.counts,
      total: agg.total,
      dominant: this._dominantOf(agg.counts, agg.lastTapSeqByEmotion)
    };
  }
  ...
  getHeatmapSlices(matchId, roomKey) {
    ...
    for (const period of periodsToInclude) {
      ...
      for (let i = 0; i <= maxSlice; i++) {
        const agg = periodAgg.get(i) || { counts: this._emptyCounts(), total: 0, lastTapSeqByEmotion: {} };
      ...
      try {
        ...
        for (const normalized of liveMatches) {
          ...
          if (!match) {
            ...
            // Rehydrate in-memory aggregates for this match from durable tap events.
            if (this._listTapEvents) {
              const persistedTaps = await this._listTapEvents(matchId);
              if (Array.isArray(persistedTaps) && persistedTaps.length > 0) {
                for (const tap of persistedTaps) {
                  this.recordTap(
                    matchId,
                    tap.roomKey || ROOM_GLOBAL,
                    tap.period || PERIOD.FIRST_HALF,
                    Number(tap.sliceIndex || 0),
                    tap.emotion
                  );
                }
                console.log(
                  `[SoccerEngine] Rehydrated ${persistedTaps.length} taps for match ${matchId}`
                );
              }
            }
            continue;
        ...
        // Reconcile matches that disappeared from livescores.
        // Sportmonks can drop full-time fixtures from /livescores before we observe an ended status.
        // For any active match that is no longer in the live feed, fetch fixture state directly and archive if ended.
        for (const [matchId, match] of this.matches.entries()) {
          if (match.archived) continue;
          if (seenLiveMatchIds.has(matchId)) continue;

          const latest = await this.provider.getMatchState(matchId, match.elapsedSeconds);
          if (!latest) continue;

          const resolvedPeriod = this._normalizePeriod(latest.periodLabel, latest.elapsedSeconds);
          const resolvedMinute = this._getAbsoluteMinute(latest.elapsedSeconds);

          match.elapsedSeconds = Math.max(match.elapsedSeconds, latest.elapsedSeconds);
          match.currentPeriod = resolvedPeriod;
          match.currentSliceIndex = this._computePeriodSliceIndex(resolvedPeriod, resolvedMinute);
          match.status = latest.status;
          match.periodLabel = latest.periodLabel;
          match.homeTeam = latest.homeTeam;
          match.awayTeam = latest.awayTeam;

          if (resolvedPeriod === PERIOD.EXTRA_TIME_1 || resolvedPeriod === PERIOD.EXTRA_TIME_2) {
            match.hasExtraTime = true;
          }
          if (resolvedPeriod === PERIOD.PENALTIES || latest.penaltyKicks > 0) {
            match.hasPenalties = true;
            match.penaltyKicks = Math.max(match.penaltyKicks || 0, latest.penaltyKicks || 0);
          }

          if (latest.status === STATUS.ENDED && !match.archived) {
            this.broadcastToMatch(matchId, 'status_change', {
              matchId,
              status: match.status,
              period: match.currentPeriod,
